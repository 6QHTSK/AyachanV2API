...
var diffLevels = [5]int{5, 11, 18, 22, 30}
...
func getLevelCompare(diffType int, diff int, value float64, totalLevel float64) (status model.DifficultyDescription) {
	if diffType == diffTypeFlickNoteInterval || diffType == diffTypeNoteFlickInterval {
		if diff <= 1 && value > 0 {
			return model.DifficultyHigh
		} else {
			return model.DifficultyNormal
		}
	}
...
func standardDifficultyGetter(metrics model.ChartMetricsStandard, diff int) (StandardDifficulty model.ChartDifficultyStandard, newDiff int) {
	newDiff = getTrueDiff(diff, metrics.TotalNPS, metrics.TotalHPS)
	StandardDifficulty = model.ChartDifficultyStandard{
...
func extendDifficultyGetter(metrics model.ChartMetricsExtend, diff int, level float64) (ExtendDifficulty *model.ChartDifficultyExtend) {
	return &model.ChartDifficultyExtend{
