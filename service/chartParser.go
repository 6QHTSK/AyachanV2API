...
func getCurrentLane(time float64, lastTick *ayachanChart.ParsedNote) (lane float64) {
...
func isBelongToSlide(note *ayachanChart.ParsedNote, pos int) bool {
	return note.Type == ayachanChart.NoteTypeSlide && note.Pos == pos
}

func checkNoteWhileSlide(note ayachanChart.ParsedNote, lastTickNote *ayachanChart.ParsedNote, hand ayachanChart.HandType) (bool, model.IrregularInfo) {
	if hand == ayachanChart.LeftHand { // (ii - 2)当前为左手绿条，右手其他键，其他键应满足lane > 绿条lane
		if note.Lane <= getCurrentLane(note.Time, lastTickNote) {
			return false, model.IrregularInfo{
				Irregular:     model.RegularTypeIrregular,
				IrregularInfo: fmt.Sprintf("%.2fs 处右手跨过左手击打的绿条/与绿条重叠", note.Time),
			}
		}
	} else if hand == ayachanChart.RightHand { // (ii - 3)当前为右手绿条，左手其他键，其他键应满足lane < 绿条lane
		if note.Lane >= getCurrentLane(note.Time, lastTickNote) {
			return false, model.IrregularInfo{
				Irregular:     model.RegularTypeIrregular,
	...
	return true, model.IrregularInfo{}
}

func labelNoteWhileSlide(note *ayachanChart.ParsedNote, AnotherHandAvailable *bool, AnotherHandPos *int, AnotherHand ayachanChart.HandType) (bool, model.IrregularInfo) {
	if *AnotherHandAvailable { // 如果另一只手未击打绿条，直接指定即可
		...
		if isBelongToSlide(note, *AnotherHandPos) { // 如果归属，就正常进行
			if note.Status == ayachanChart.SlideEnd {
...
func checkIfLabeled(note ayachanChart.ParsedNote) bool {
	return note.Hand != ayachanChart.LeftHand && note.Hand != ayachanChart.RightHand
}

// parseChart 拆谱拆谱、输入的谱排序好先
func parseChart(Map ayachanChart.AyachanChart) (parsedMap ayachanChart.ParsedChart, irregularInfo model.IrregularInfo) {
	defer func() {
		err := recover()
		if err != nil {
			// config.Log.Errorf("分析谱面出现异常！异常情况：%s\n", err)
			parsedMap = nil
			irregularInfo = model.IrregularInfo{
				Irregular:     model.RegularTypeUnknown,
	...
	for i := 2; i < mapLen; i++ {
		if parsedMap[i].Beat == parsedMap[i-1].Beat && parsedMap[i-1].Beat == parsedMap[i-2].Beat {
			return parsedMap, model.IrregularInfo{
				Irregular:     model.RegularTypeIrregular,
			...
			if parsedMap[i].Lane == parsedMap[i-1].Lane {
				...
				parsedMap[i].Hand = ayachanChart.RightHand
				parsedMap[i-1].Hand = ayachanChart.LeftHand
			}
		}
	}
	...
	for i, note := range parsedMap {
		...
		if note.Type == ayachanChart.NoteTypeSlide && note.Status == ayachanChart.SlideStart {
			// 该绿条还未被标记
			if note.Hand == ayachanChart.UnknownHand {
				//1.串联整个绿条
				hand := ayachanChart.UnknownHand
				lastTick := i
				var lastNoteBeat float64
				for j := i + 1; j < parsedMap.Len(); j++ {
					if parsedMap[j].Type == ayachanChart.NoteTypeSlide && parsedMap[j].Pos == note.Pos {
						...
						lastTick = j
						if parsedMap[j].Status == ayachanChart.SlideEnd {
				...
				AnotherHand := ayachanChart.UnknownHand // AnotherHand 记录另一只手是什么
				...
				for j := i - 1; j < parsedMap.Len(); j++ {
					...
					// （5）第一次出现其他Note，记录下来
					if hand == ayachanChart.UnknownHand {
						currentLane := getCurrentLane(parsedMap[j].Time, lastTickNote)
						if parsedMap[j].Lane < currentLane {
							hand = ayachanChart.RightHand
							AnotherHand = ayachanChart.LeftHand
						} else if parsedMap[j].Lane > currentLane {
							hand = ayachanChart.LeftHand
							AnotherHand = ayachanChart.RightHand
						} else {
							return parsedMap, model.IrregularInfo{
								Irregular:     model.RegularTypeIrregular,
				...
				// 2. 遍历持续范围内的所有键
				var lastTickNote *ayachanChart.ParsedNote // lastTickNote 记录上一个绿条节点，便于推算了绿条当前位置
				var AnotherHand ayachanChart.HandType     // AnotherHand 记录另一只手是什么
				if hand == ayachanChart.LeftHand {
					AnotherHand = ayachanChart.RightHand
				} else if hand == ayachanChart.RightHand {
					AnotherHand = ayachanChart.LeftHand
				}
				AnotherHandAvailable := true // AnotherHandAvailable 记录另一只手是否被绿条占用
				AnotherHandPos := 0          // 另一只手被绿条占用时的绿条编号Pos
		...
		if checkIfLabeled(parsedMap[i]) &&
			...
			if interval1 < 0.2 && interval2 < 0.2 &&
				...
				if parsedMap[i-1].Lane > parsedMap[i].Lane &&
					...
					parsedMap[i].Hand = ayachanChart.TryLeftHand
					parsedMap[i-1].Hand = ayachanChart.TryRightHand
					parsedMap[i-2].Hand = ayachanChart.TryLeftHand
				}
				...
				if parsedMap[i-1].Lane > parsedMap[i].Lane &&
					...
					parsedMap[i].Hand = ayachanChart.TryRightHand
					parsedMap[i-1].Hand = ayachanChart.TryLeftHand
					parsedMap[i-2].Hand = ayachanChart.TryRightHand
				}
			}
		}
	}
	// 第四部分，全盘采用第三部分的拆分建议
	for i := range parsedMap {
		if parsedMap[i].Hand == ayachanChart.TryLeftHand {
			parsedMap[i].Hand = ayachanChart.LeftHand
		} else if parsedMap[i].Hand == ayachanChart.TryRightHand {
			parsedMap[i].Hand = ayachanChart.RightHand
		}
	}

	// 第五部分，对其他部分采用贪心算法拆谱。可以证明，未标记的Note均为单个独立的出现，直接根据左手、右手的最终note就行了。连接左右手的Note
	var lastLeftHandNote, lastRightHandNote *ayachanChart.ParsedNote
	for i := range parsedMap {
		// 如果还没有指定哪个手
		if parsedMap[i].Hand == ayachanChart.UnknownHand {
			//Case 1 那只手近用哪只手
	...
	return parsedMap, model.IrregularInfo{Irregular: model.RegularTypeRegular}
}
